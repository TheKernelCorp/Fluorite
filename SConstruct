...
target = {
  'arch': GetOption('arch'),
  'host': GetOption('host'),
  'triple': '{}-unknown-fluorite-none'.format(GetOption('arch')),
}
...
files = {
  ...
  'dst': {
    ...
    'kernel': os.path.join(dirs['iso:boot'], 'fluorite.kern'),
    'iso': '{build}/fluorite-{arch}.iso'.format(build=dirs['build'], arch=target['arch']),
    'boot:kernel': '{sysroot}/kernel.o'.format(sysroot=dirs['sysroot:boot']),
  },

  # Object files
  'obj': {
    'kernel': '{build}/fluorite-{arch}.o'.format(build=dirs['build'], arch=target['arch']),
  },
